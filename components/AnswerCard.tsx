  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-4">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs px-2.5 py-0.5 rounded-full border font-medium ${badgeInfo.style}`}>
            {badgeInfo.label}
          </span>
          {coverage && (
            <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
              Coverage: {coverage.expertsCovered}/{coverage.totalExperts || 3} Experts
            </span>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-800 leading-relaxed font-normal bg-slate-50 p-4 rounded-lg border border-slate-200">
            {answer}
          </p>
        </div>
      ) : (
        <p className="text-sm text-slate-400 italic">
          Click &ldquo;Analyze Question&rdquo; to execute grounded synthesis.
        </p>
      )}
    </div>
  );
